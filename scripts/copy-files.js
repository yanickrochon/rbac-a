 ...
 * like imports who are tree-shakeable.
 *
...
async function createModulePackages({ from, to }) {
  ...
  await Promise.all(
    directoryPackages.map(async directoryPackage => {
      ...
      // do not care about TypeScript right now...
      //if (!typingsExist) {
...
async function addLicense(packageData) {
  const license = `/** @license RBAC-A v${packageData.version}
 *
...
async function run() {
  try {
    ...
    await Promise.all(
      [
        // use enhanced readme from workspace root for `RBAC-A`
        './README.md',
        './LICENSE',
