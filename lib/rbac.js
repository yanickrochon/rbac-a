...
function filterRolesByAttributes(emitter, user, roles, params, provider, attributes) {
  ...
  function _validate(roles) {
    return Promise.all(Object.keys(roles).map(function (role) {
      return attributeCache[role] || (attributeCache[role] = Promise.resolve().then(function () {
        ...
        return roleAttributes ? Promise.all(roleAttributes.map(function (attrName) {
          return Promise.resolve().then(function () {
            return attributes.validate(attrName, attributeContext);
          }).then(function (valid) {
            if (valid) {
              activeAttributes.push(attrName);
            }
        ...
        err = err || new Error('AttributeManager error');
        err.user = user;
        err.role = role;
      ...
      return Promise.all(activeRoles.map(function (role) {
        if (role) {
          if (roles[role]) {
            return _validate(roles[role]).then(function (nextRoles) {
              filteredRoles[role] = nextRoles;
            });
          } else {
            filteredRoles[role] = null;  
          }
        }
      })).then(function () {
  ...
  return _validate(roles); // returns filtered roles
}
